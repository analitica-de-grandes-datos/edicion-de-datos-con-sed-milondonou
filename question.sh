...
#
sed 's/\([0-9][0-9]\)\/\([0-9][0-9]\)\/\([0-9][0-9]\)/20\3-\2-\1/' data.csv | sed 's/a/A/' | sed 's/b/B/' | sed 's/c/C/' | sed 's/n/N/' | sed 's/;N;/;\\N;/' | sed 's/;N/;\\N/' | sed 's/;;/;\\N;/' | sed 's/20\([0-9][0-9]\)-\([0-9][0-9]\)-\([0-9][0-9]\);\\N;\\N;/20\1-\2-\3;\\N;\\N;\\N/' | sed 's/20\([0-9][0-9]\)-\([0-9][0-9]\)-\([0-9][0-9]\);C;\\N;/20\1-\2-\3;C;\\N;\\N/' | sed 's/;\\N\\N/;\\N/' | sed 's/,/./' | sed 's/;/,/ g' | sed 's/\([0-9]\)\/\([0-9]\)\/20\([0-9][0-9]\)/20\3-0\2-0\1/'